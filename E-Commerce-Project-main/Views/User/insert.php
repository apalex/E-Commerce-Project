...
if ($conn -> connect_error) {
    ...
    if (empty($F_NAME)) {
        ...
        $errors['first_name'] = "2 Letters minimum required";
    }
    if (empty($L_NAME)) {
        ...
        $errors['last_name'] = "2 Letters minimum required";
    }
    if (empty($EMAIL)) {
    ...
    if(!empty($PASSWD) && !empty($C_PASSWD)) {
        if (!preg_match("^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$", $PASSWD)) {
            $errors['password'] = "8 characters minimum, 1 uppercase letter, 1 lowercase letter, 1 special character required";
